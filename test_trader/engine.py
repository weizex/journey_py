...
"""engine作为一种功能的封装，内聚，内部联系紧密互相间低耦合
baseengine是基类，共用一个eventengine用于处理事件
各个功能的engine注册到mainengine中统一管理"""
EngineType = TypeVar("EngineType", bound="BaseEngine")
'''值得注意的是，python本身也具有向下转换的能力，把sub对象或类传入base没问题
但注意，这种方式下，python本身是对参数的类型进行了模糊化的判断的，而typevar则完全保留了
原参数的类型，不被转换，这在一些情况下是必要的，比如需要return sub类时
'''
...
class OmsEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        self.ticks: dict[str, TickData] = {}#合约到行情快照
        self.orders: dict[str, OrderData] = {}
        self.trades: dict[str, TradeData] = {}#交易所的成交数据
        self.positions: dict[str, PositionData] = {}#持仓状态
        self.accounts: dict[str, AccountData] = {}
        self.contracts: dict[str, ContractData] = {}#合约静态信息
        self.quotes: dict[str, QuoteData] = {}#报价

        self.active_orders: dict[str, OrderData] = {}#活跃进行中订单
        self.active_quotes: dict[str, QuoteData] = {}#活跃中报价

        self.offset_converters: dict[str, OffsetConverter] = {}#平仓偏移规则
...
class MainEngine:
    ...
    def add_engine(self, engine_class: type[EngineType]) ->EngineType:
        engine: EngineType = engine_class(self, self.event_engine)#engine需要传入mainengine，engine——name在baseengine构造时赋值
    ...
    def init_engines(self) -> None:
        ...
        #将上层的命令与其拥有的下层命令绑定，从而使用下层提供的 方法是常用的方式
